...
function ChromeListeners() { 
  ...
  this.onTabUpdated = function(tab_id, change_info, tab) {
    if(change_info.url != undefined) {
      supertabs_api.supertabs.updateTab(tab.windowId, tab.id, tab.url);
      supertabs_api.update_tab(supertabs_api.supertabs.getSupertabTabByChromeId(tab.windowId, tab.id));
    }
  };
  ...
  this.onTabRemoved = function(tab_id) {
    var found = supertabs.deleteTab(tab_id);
    if (found.restored) {
      supertabs_api.undelete();
    } else {
      var tab = found.getJSONObject();
      tab.supertab_id = found.parent_id;
      supertabs_api.delete_tab(tab);
    }
  };
